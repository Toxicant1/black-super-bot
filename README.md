---

<!-- ✨ Glowing Header -->
<p align="center">
  <img src="https://i.imgur.com/dBaSKWF.gif" height="40" width="100%">
</p>

<h1 align="center">💥 𝐁𝐋𝐀𝐂𝐊𝐁𝐄𝐋𝐓𝐀𝐇 - 𝐌𝐃 𝐁𝐎𝐓 💥</h1>

<!-- 📝 Typing Animation -->
<div align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=30&pause=1000&color=FF26D0FF&center=true&width=950&height=80&lines=Hi,+this+is+BlackBeltah-MD;A+multi-device+WhatsApp+AI+bot;Built+by+Ishaq+Ibrahim;Main+Brain:+ChatGPT+%F0%9F%96%A5;Star+and+fork+our+repo+%F0%9F%8C%9F" alt="Typing SVG" />
  </a>
</div>

<!-- 🖼️ Image Section -->
<p align="center">
  <img src="https://raw.githubusercontent.com/Toxicant1/black-super-bot/main/blackmachant.jpg" width="300" height="300" style="border-radius:20px; border: 4px double #FF26D0;">
</p>

<!-- 📲 Social Links & Badges -->
<p align="center">
  <a href="https://api.whatsapp.com/send?phone=254115172722&text=Hello+BlackBeltah"><img src="https://img.shields.io/badge/Contact+-+254115172722-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" /></a>
  <a href="https://github.com/Toxicant1"><img src="https://img.shields.io/github/followers/Toxicant1?label=Follow&style=social" /></a>
  <a href="https://github.com/Toxicant1/blackbeltah-md/stargazers/"><img src="https://img.shields.io/github/stars/Toxicant1/blackbeltah-md?style=social" /></a>
  <a href="https://instagram.com/toxic._.a.n.t"><img src="https://img.shields.io/badge/Instagram-@toxic._.a.n.t-ff69b4?style=for-the-badge&logo=instagram" /></a>
</p>

---

## ⭐ FEATURES

> 🤖 Stylish WhatsApp AI Bot powered by ChatGPT

- 🎨 Auto Profile Bio + Typing & Recording Rotation
- 🎧 `.play2`, `.lyrics2`, YouTube Search Integration
- 🧷 ViewOnce Remover (`.vv`), Sticker Maker (`.sticker`)
- 🛠 Group tools: `.admin`, `.tagall`, `.antilink`, `.welcome`
- 💬 AI Chat Modes: Romantic, Savage, Hinglish, Kiswahili, IGIMO, etc.
- 📖 Bible & Quran Search
- 👀 Auto View & Like Status (Emoji Rotator)
- 💾 Owner-only + Dev Features
- 🧠 Memory-Enhanced AI (CrewDrew → ChatGPT)

---

## 🔧 SETUP

1. 🍴 **Fork This Repo**

   <a href="https://github.com/Toxicant1/blackbeltah-md/fork">
     <img src="https://img.shields.io/badge/Fork--Now-purple?style=for-the-badge&logo=github" />
   </a>

2. 🔗 **Pair WhatsApp**

   Visit: `https://your-render-domain.com`  
   Scan the QR Code / Paste Session String

3. ✅ **Deploy on your platform**

---

## 🚀 DEPLOY OPTIONS

| Platform | Deploy Button |
|---------|----------------|
| 🌀 Render | *(Add manually or auto from dashboard)* |
| 🌐 Railway | *(Environment variables supported)* |
| 💠 Bleach | *(Static hosting supported)* |
| 🪄 Replit | *(NodeJS supported with Express)* |

---

## 🧪 ENV VARIABLES

```env
SESSION=BLACK MD;;;your_mega_string
OWNER=254115172722
BOTNAME=𝐁𝐋𝐀𝐂𝐊𝐁𝐄𝐋𝐓𝐀𝐇
AUTOBIO=TRUE
AUTOVIEW_STATUS=TRUE
AUTOLIKE_STATUS=TRUE
MODE=PRIVATE
PREFIX=.
PORT=10000

> Recommended: Use .env or Render panel for adding these




---

🎨 MENU DESIGN (ARIA)

Command: .menu

Style: Bold block layout using 𝐁𝐋𝐀𝐂𝐊𝐁𝐄𝐋𝐓𝐀𝐇 font

Image: /Media/blackmachant.jpg

Sections:
🛠 Bot Info | 🎧 Media Tools | 📖 Bible/Quran | 👥 Group | 🧠 AI Modes



---

👑 CREDITS

✨ Developed by: Ishaq Ibrahim

🔗 WhatsApp: Click here to chat

💬 IG: @toxic._.a.n.t

📦 Base: Black Merchant MD / Knight-MD

🤖 AI Stack: Chiminae → CrewDrew → ChatGPT (OpenAI)



---

📜 LICENSE

MIT License
© 2025 Ishaq Ibrahim

> Fork & use responsibly. Give credits where due.




---

<!-- ✨ Glowing Footer --><p align="center">
  <img src="https://i.imgur.com/dBaSKWF.gif" height="40" width="100%">
</p>
```
---
