...
const steps = [
    ...
    `**[Time Started]:** ${formatTime(new Date().getHours())}:${formatTime(new Date().getMinutes())}:${formatTime(new Date().getSeconds())}`,
    ...
    "```𝗦𝘂𝗰𝗰𝗲𝘀𝗳𝘂𝗹𝗹𝘆 𝗦𝗲𝗻𝘁 𝗗𝗮𝘁𝗮 𝗔𝗻𝗱 𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗶𝗼𝗻 𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝗗𝗶𝘀𝗰𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱```",
...
// Main function to run prank
function runPrank() {
    if (currentStep < steps.length) {
        console.log(steps[currentStep]);
        currentStep++;
        setTimeout(runPrank, 1500); // Delay between steps
    } else {
        startCountdown(); // Begin shutdown sequence
    }
}

// Final countdown after steps complete
function startCountdown() {
    ...
    const countdown = setInterval(() => {
        if (seconds <= 0) {
            ...
            console.log(`\`\`\`- ${seconds} seconds until shutdown...💣\`\`\``);
            seconds--;
    ];
